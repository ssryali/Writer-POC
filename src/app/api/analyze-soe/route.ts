...
const WRITER_APP_ID = "20d7cfa8-7cc5-47b7-80c3-96a6e4d7c81f";
...
export async function POST(request: Request) {
  try {
    ...
    const response = await fetch(`https://api.writer.com/v1/applications/${WRITER_APP_ID}`, {
      ...
      body: JSON.stringify({
        inputs: [{ id: "Text input", value: [strategicInput] }],
      }),
    });
    ...
    return NextResponse.json({
      ...
      agentId: "US-MKT-01",
      agentName: "Single Objective Enforcement",
    });
  } catch (error) {
    console.error("[ANALYZE-SOE] Error:", error instanceof Error ? error.message : error);
    return NextResponse.json(
